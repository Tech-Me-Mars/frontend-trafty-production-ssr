...
const schema = yup.object({
  old_password: yup.string().required(t('กรุณากรอกรหัสเดิม')),
  new_password: yup.string()
    .required(t('กรุณากรอกรหัสใหม่'))
    .min(8, `${t('ต้องมีอย่างน้อย')} 8 ${t('ตัว')}`)
    .max(20)
    .matches(/^(?=.*[a-zA-Z])(?=.*\d)(?=.*[\W_]).+$/, t('ต้องมีทั้งตัวอักษร ตัวเลข และสัญลักษณ์')),
  confirm_password: yup.string()
    .required(t('กรุณายืนยันรหัสผ่านใหม่'))
    .oneOf([yup.ref('new_password')], t('รหัสผ่านไม่ตรงกัน'))
})
...
<template>
  <form @submit.prevent="onSubmit">
    <div class="min-h-screen bg-gray-100 pb-24">
      <LayoutsBaseHeader :title="t('แก้ไขรหัสผ่าน')" :showIcon="false" :showBack="true" :close="false"
      :showMenu="false"></LayoutsBaseHeader>

      <div class="bg-white rounded-xl shadow-sm p-6 mx-4 mt-6 space-y-6">
        <h2 class="text-lg font-bold text-gray-800">{{ t('รหัสผ่าน') }}</h2>
        <p class="text-sm text-gray-600 leading-5">
          {{ t('รหัสผ่านต้องมีความยาว') }} 8–20 {{ t('ตัวอักษร') }}<br />
          {{ t('ประกอบด้วยตัวอักษร ตัวเลข และสัญลักษณ์') }}
        </p>
        ...
        <div>
          <FloatLabel>
            <Password
              ...
              :feedback="false"
            />
            <label for="old_password" class="label-input">{{ t('รหัสเดิม') }}</label>
          </FloatLabel>
        ...
        <div>
          <FloatLabel>
            <Password
              ...
              :feedback="false"
            />
            <label for="new_password" class="label-input">{{ t('รหัสผ่านใหม่') }}</label>
          </FloatLabel>
        ...
        <div>
          <FloatLabel>
            <Password
              ...
              :feedback="false"
            />
            <label for="confirm_password" class="label-input">{{ t('ยืนยันรหัสผ่านใหม่') }}</label>
          </FloatLabel>
