<template>
  <div class="mt-4">
    ...
    <swiper class="mt-4" :slides-per-view="4" space-between="20" pagination @slideChange="onSlideChange" :breakpoints="{
      768: { slidesPerView: 5 },
      1024: { slidesPerView: 6 }
    }">
      <!-- 1 slide = 2 items บนล่าง -->
      <swiper-slide v-for="(pair, index) in groupedCategory" :key="index" class="flex flex-col items-center space-y-4">
        <div v-for="item in pair" :key="item.id" class="flex flex-col items-center text-center">
          <NuxtLink :to="`/client/category/${item.id}`" :style="{ backgroundColor: item.bg_color }"
    ...
    <div class="max-w-[3rem] mx-auto">
      <div class="relative w-full h-1.5 bg-gray-300 rounded-full mt-4">
        <div class="absolute top-0 left-0 h-1.5 bg-[#281c74] rounded-full transition-all"
