<template>
  <div class="min-h-screen bg-zinc-50">
    <LayoutsBaseHeader :title="t('ตรวจสอบคอมเม้นต์')" :showBack="true" />
    ...
    <section class="px-4 pt-3">
      <div class="bg-white rounded-lg border border-gray-200 p-3 flex items-center justify-between">
        ...
        <Dropdown v-model="selectedStar" :options="starOptions" optionLabel="label" optionValue="value"
          :placeholder="t('เลือกคะแนน')" class="w-48" :loading="isLoading" showClear @change="loadComments" />
      </div>
    </section>

    <!-- Skeleton while loading -->
    <div v-if="isLoading" class="p-4 space-y-4">
      <div v-for="i in 3" :key="i" class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
...
const toggleReviewVisibility = async (newVal) => {
  ...
  try {
    ...
    // แจ้งเตือน
    toast.value = { show: true, type: 'danger', title: t('ผิดพลาด'),
