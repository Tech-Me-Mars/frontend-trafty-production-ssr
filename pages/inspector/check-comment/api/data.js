...
export const getCommentByStar = async (star = 0) => {
  const modulepath = await getModulePathByRoleName()

  return request('GET', `/api/v1/${modulepath}/business_comment/get-comment-by-star/${star}`,{},true);
};

// อัปเดตสถานะการแสดงผลของคอมเมนต์
export const updateCommentVisibility = async (commentId, status) => {
  const modulepath = await getModulePathByRoleName()
  ...
  return request('PUT', `/api/v1/${modulepath}/business_comment/${commentId}`, payload, true);
};
